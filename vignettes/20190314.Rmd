...
rawfiles <- rawfiles[!grepl("Blank_", rawfiles)]
rawfiles <- rawfiles[grepl("pos_", rawfiles)]
...
X <- lapply(rawfiles, function(rawfile){
 rv <- try(uvpd:::.extractXICs(rawfile, smiles=ThermoUVPD_feb2019$SMILES))
})
#dev.off()
...
#SS <- do.call('rbind', S)
lapply(S[6:15], function(x){
  if(is.null(x)){return(NULL)}
xyplot(log(intensity,10) ~ mZ | SMILES0 * basename(rawfile),
       groups = type, 
       data=x, 
       type='h')
})
