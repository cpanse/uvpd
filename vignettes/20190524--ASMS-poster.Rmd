...
```{r XIC, fig.retina=3, fig.path='./graphics/'}
library(ggplot2)
plot(XIC) + labs(x = "Retention Time [min]", y = "Intensity")
```
...
### Plot in-silico spec
```{r in-silico-fragmentIons, fig.retina=3, fig.path='./graphics/'}
plot(0,0, xlim=range(f.mZ<-unique(df.frags$mZ)), ylim=c(0,1), type = 'n', xlab = 'mZ', ylab=''); abline(v=df.frags$mZ, col=df.frags$type)
```

## Castell
```{r Castell, fig.retina=3, fig.height=12, fig.width=19, fig.path='./graphics/'}
