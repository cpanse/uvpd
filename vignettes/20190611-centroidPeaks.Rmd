...
```{r}
Smatch <- do.call('rbind', lapply(uvpd.Castell, function(x){aggregate(intensity ~ SMILES0 * rawfile * scan, data=x, FUN=length)}))

names(Smatch) <- c("SMILES0",   "rawfile",   "scan",      "nPSM")
