...
fragmentsPredicted <- as.data.frame(do.call('rbind', lapply(fragments.treeDepth2$ms2, function(x){table(x$type)})))

fragmentsPredicted$formula <- fragments.treeDepth2$formula
```
...
.stackedbarcharts <- function(compound){
    ...
    gp <- ggplot(data = DF[DF$compound == compound, ], 
        ...
        facet_wrap(~ compound * Group * mode, scales="free")
    gp
}

.stackedbarcharts(compound = "Flubendazole")
```
