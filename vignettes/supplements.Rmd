...
X.top3.master.intensity.MS2.centroid <- lapply(sort(unique(X.top3.master.intensity$file)), function(f){
    sn <- X.top3.master.intensity$scan[X.top3.master.intensity$file==f]
...
dim(X)
Y <- do.call('rbind', do.call('rbind', X.top3.master.intensity.MS2.centroid))
Y <- Y[Y$eps < 0.01,]
dim(Y)
...
lapply(unique(DF$compound), function(compound){
